...
function init() {
  ...
  for (counter = 0; counter < 8; counter++) {
    ...
    local.sendPitchWheel(
      counter + 1,
      local.values.strips.getChild("Strip " + (counter + 1)).faderValue.get() *
        16383
    );
    //Pulse VU Meters at current value
    local.sendChannelPressure(
      counter + 1,
      local.values.strips.getChild("Strip " + (counter + 1)).meter.get() * 14 +
        16 * counter
    );
    //Init POT LEDs
    if (
      (local.values.strips.getChild("Strip " + (counter + 1)).rotaryMode.get() -
        1) /
        16 ==
        3 ||
      (local.values.strips.getChild("Strip " + (counter + 1)).rotaryMode.get() -
    ...
    local.sendNoteOn(
      1,
      counter + 16,
      local.values.strips.getChild("Strip " + (counter + 1)).mute.get()
    );
    //Init Solo
    local.sendNoteOn(
      1,
      counter + 8,
      local.values.strips.getChild("Strip " + (counter + 1)).solo.get()
    );
    //Init Rec
    local.sendNoteOn(
      1,
      counter + 0,
      local.values.strips.getChild("Strip " + (counter + 1)).rec.get()
    );
    //Init Select
    local.sendNoteOn(
      1,
      counter + 24,
      local.values.strips.getChild("Strip " + (counter + 1)).select.get()
    );
  }
...
function updateStripArray() {
  for (counter = 0; counter < 8; counter++) {
    stripArray[counter] = fixedLengthString(
      local.values.strips.getChild("Strip " + (counter + 1)).faderName.get(),
      14
    );
    encoderArray[counter] = fixedLengthString(
      local.values.strips.getChild("Strip " + (counter + 1)).encoderName.get(),
      14
    );
...
function updateScribleColor() {
  ...
  for (counter = 0; counter < 8; counter++) {
    colorArray[counter] += parseInt(
      local.values.strips.getChild("Strip " + (counter + 1)).displayColor.get()
    );
  }
...
function update(deltaTime) {
  ...
  if (
    hours !=
    Math.floor(
      Math.floor((((UTCStamp + UTCOffset) % yearSecs) % daySecs) / hourSecs)
    )
  ) {
    hours = Math.floor(
      Math.floor((((UTCStamp + UTCOffset) % yearSecs) % daySecs) / hourSecs)
  ...
  if (
    ...
    Math.round(
      Math.floor(
        ((((UTCStamp + UTCOffset) % yearSecs) % daySecs) % hourSecs) /
          minuteSecs
      )
    )
  ) {
...
function moduleParameterChanged(param) {
  if (param.isParameter()) {
    ...
    if (param.name == "flashOnSolo") {
      var i;
      for (i = 0; i < 8; i++) {
        if (local.values.strips.getChild("Strip " + (i + 1)).solo.get() == 1) {
          local.values.strips.getChild("Strip " + (i + 1)).solo.set("on");
        } else {
          if (
            local.values.strips.getChild("Strip " + (i + 1)).solo.get() == 127
          ) {
            local.values.strips.getChild("Strip " + (i + 1)).solo.set("flash");
          }
        }
      }
    }
    ...
    if (value.name == "faderValue") {
      ...
      if (value.name == "meter") {
        local.sendChannelPressure(
          1,
          value.get() * 14 +
            (parseInt(value.getParent().name.substring(5, 6)) - 1) * 16
        );
      } else {
        if (value.name == "select") {
          local.sendNoteOn(
            1,
            parseInt(value.getParent().name.substring(5, 6)) + 23,
            value.get()
          );
        } else {
            ...
            if (value.name == "encoderName") {
              ...
              if (value.name == "faderName") {
                ...
                if (value.name == "solo") {
                  local.sendNoteOn(
                    1,
                    parseInt(value.getParent().name.substring(5, 6)) + 7,
                    value.get()
                  );
                } else {
                  if (value.name == "mute") {
                    local.sendNoteOn(
                      1,
                      parseInt(value.getParent().name.substring(5, 6)) + 15,
                      value.get()
                    );
                  } else {
                    if (value.name == "rec") {
                      local.sendNoteOn(
                        1,
                        parseInt(value.getParent().name.substring(5, 6)) - 1,
                        value.get()
                      );
                    } else {
...
function noteOnEvent(channel, pitch, velocity) {
  ...
  if (pitch >= 8 && pitch <= 15) {
    ...
    if (local.values.strips.getChild("Strip " + (index + 1)).solo.get() == 0) {
      if (local.parameters.flashOnSolo.get()) {
        local.values.strips.getChild("Strip " + (index + 1)).solo.set("flash");
      } else {
  ...
  if (pitch >= 32 && pitch <= 39) {
    var index = pitch - 32;
    if (velocity == 127) {
      local.values.strips.getChild("Strip " + (index + 1)).push.set(1);
    }
  }
...
function noteOffEvent(channel, pitch, velocity) {
  ...
  if (pitch >= 32 && pitch <= 39) {
    var index = pitch - 32;
    if (velocity == 0) {
      local.values.strips.getChild("Strip " + (index + 1)).push.set(0);
    }
  }
}
...
function pitchWheelEvent(channel, value) {
  ...
  //It's a strip fader
    ...
    local.values.strips
      .getChild("Strip " + channel)
      .faderValue.set(value / 16380);
    updateScribble();
  }
}
...
function setVuMeterValue(strip, input, rangeStart, rangeEnd) {
  //rangeStart=0;
  //rangeEnd=100;
  //inp = map(input,rangeStart,rangeEnd,0,1);
  local.values.strips.getChild("Strip " + strip).meter.set(input);
}
